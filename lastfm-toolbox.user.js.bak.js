...
/* Minimal Icon Style Overrides */
    ...
    /* Highlight Eligible Links (Only applies when icons are hidden AND setting is enabled) */
    html.lfm-highlight-eligible.lfm-hide-hover-icons a.lfm-toolbox-enabled:hover {
      color: var(--brand) !important;
...
.service-launch-btn svg {
  ...
  if (localStorage.getItem("setting-icon-style") === "minimal") {
    document.documentElement.classList.add("lfm-icon-style-minimal");
  }
...
*Data from TheAudioDB • Last updated via API*`;
  ...
  function setupToggles() {
    ...
    if (aiPopupToggle) {
      ...
      if (isPopup) {
        aiPopupToggle.classList.add("active");
        aiPopupToggle.dataset.text = "Popup";
      } else {
        aiPopupToggle.classList.remove("active");
        aiPopupToggle.dataset.text = "Tab";
      }
      aiPopupToggle.onclick = function () {
        const active = this.classList.toggle("active");
        localStorage.setItem("setting-ai-popup", active ? "true" : "false");
        this.dataset.text = active ? "Popup" : "Tab";
      };
    }
      ...
      <div id="lfm-modal-box" class="settings-modal">
        ...
        <div class="settings-panel" data-panel="general">
          <div class="settings-group">
            <div class="settings-item">
              <div class="settings-info">
                <span class="settings-label">Show Inline Action Icons</span>
                <span class="settings-desc">Displays the small Last.fm toolbox button next to eligible artist links and image cards.</span>
              </div>
              <div id="modal-toggle-artist-symbol" class="toggle-switch active"></div>
            </div>
            <div class="settings-item">
              <div class="settings-info">
                <span class="settings-label">Inline Icon Style</span>
                <span class="settings-desc">Choose between the dark bubble background or a minimal floating icon.</span>
              </div>
              <select id="modal-icon-style" aria-label="Icon Style">
            ...
            <div class="settings-item">
              <div class="settings-info">
                <span class="settings-label">Highlight Eligible Links on Hover</span>
                <span class="settings-desc">When inline icons are disabled, eligible text links turn red on hover to indicate right-click capability.</span>
              </div>
              <div id="modal-toggle-highlight" class="toggle-switch"></div>
            </div>
            <div class="settings-item">
              <div class="settings-info">
                <span class="settings-label">Show "Open All" Button</span>
            ...
            <div class="settings-item">
              <div class="settings-info">
                <span class="settings-label">Enable Light Mode</span>
                <span class="settings-desc">Uses a brighter interface designed for daytime browsing.</span>
              </div>
              <div id="modal-toggle-light-mode" class="toggle-switch"></div>
            </div>
            <div class="settings-item">
              <div class="settings-info">
                <span class="settings-label">Link Opening Behavior</span>
                <span class="settings-desc">Popup opens a centered window; Tab opens a regular background browser tab.</span>
              </div>
              <select id="modal-toggle-open-mode" aria-label="Open Mode">
  ...
  function setupToggles() {
    // 1. Inline Hover Icons
    const hoverToggle = document.getElementById("modal-toggle-artist-symbol") || document.getElementById("toggle-artist-symbol");
    if (hoverToggle) {
      const isHidden = localStorage.getItem("setting-artist-symbol") === "false";
      hoverToggle.classList.toggle("active", !isHidden);
      document.documentElement.classList.toggle("lfm-hide-hover-icons", isHidden);
